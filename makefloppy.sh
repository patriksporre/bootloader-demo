...
#
# Example:
#   ./makefloppy.sh boot.bin packed_application.bin floppy.img
#
...
#   3. Writes the packed application to the second sector (sector 2)
...
# Create a blank 1.44 MB floppy disk image (2 880 sectors)
...
# Write the packed application to the second sector (starting at sector 2)
dd if="$APPLICATION" of="$OUTPUT" bs=512 seek=2 conv=notrunc
